...
index = rbind(cbind(train_index, cv = "train"), cbind(test_index, cv = "test")) %>% 
  as.data.frame() %>% 
  rename(index = "train_index") %>% 
  mutate(index = as.numeric(index))

larch_data %>%
  tibble::rownames_to_column(var = "index") %>%
  mutate(index = as.numeric(index)) %>%
  left_join(index, by = "index") %>%
  ...
  labs(x = "x coordinate", y = "y coordinate") +
  theme(legend.position = "bottom") +
  ggtitle("Heatmap for Larch")


pdf("output/heatmap_larch_cv.pdf", height = 4)
larch_data %>%
  tibble::rownames_to_column(var = "index") %>%
  mutate(index = as.numeric(index)) %>%
  left_join(index, by = "index") %>%
...
run_MCMC <- function(seed, 
  ...
  spatial_model_code <- nimbleCode({
    ...
    for (i in 1:N) {
      ...
      log(phi[i]) <- psi[1] + 
        psi[2]*(mu[i]-mu_mean) +
        psi[3]*(mu[i]-mu_mean)^2 +
        psi[4]*(mu[i]-mu_mean)^2
  ...
  init_fun <- function(){
    inits <- list(b = rnorm(N_basis, 0, 0.1),
                  lambda = rep(1, 2),
                  psi = c(rnorm(1,2,sd=0.1),rnorm(3,0,0.1)))
    return(inits)
  }
...
save(samples, file = paste0('output/larch_mcmc_output-polynomial.RData'))
...
gelman_psi <- gelman.diag(lapply(samples, subset_psi), 
                          transform = TRUE, 
                          autoburnin = FALSE, 
                          multivariate = FALSE)
gelman_psi
...
traplot(samples_mcmc[, "psi[1]"], main = "psi_1")
traplot(samples_mcmc[, "psi[2]"], main = "psi_2")
traplot(samples_mcmc[, "psi[3]"], main = "psi_3")
traplot(samples_mcmc[, "psi[4]"], main = "psi_4")
...
pdf("output/heatmap_larch-posterior-train_polynomial.pdf")
mu_posterior_median %>%
  ggplot(., aes(x = X_coord, y = Y_coord, fill = mu_posterior_median)) +
  geom_tile() +
  scale_fill_viridis_c() +
...
mu_mean_train <- apply(mu_samples,1,mean)


phi_samples <- matrix(nrow = N_samples, ncol = N_train)
for(j in 1:N_train){
  mu_mean <- apply(mu_samples,1,mean)
  phi_samples[,j] <- exp(psi_samples[,1] +
                           psi_samples[,2]*(mu_samples[,j]-mu_mean) +
                           psi_samples[,3]*(mu_samples[,j]-mu_mean)^2 +
                           psi_samples[,4]*(mu_samples[,j]-mu_mean)^3)
}
phi_samples
...
save(simulation_replicates, file = 'output/replicates_polynomial.RData')
...
pdf("output/posterior_density_mean-polynomial.pdf")
simulation_replicates %>%
  apply(.,2, mean) %>%
  as.data.frame() %>%
  rename(mean = ".") %>%
...
pdf("output/posterior_density_sd-polynomial.pdf")
simulation_replicates %>%
  apply(.,2, sd) %>%
  as.data.frame() %>%
  rename(sd = ".") %>%
...
pdf("output/quantile_plot-polynomial.pdf")
quantile_summary %>%
  ggplot(., aes(x = quantile, y = median)) +
  geom_point(aes(shape = "Replicates Median"), size = 2) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = "95% Interval"), alpha = 0.2) +
...
mu_mean_test <- apply(mu_test,1,mean)

phi_test <- matrix(nrow = N_samples, ncol = N_test)
for(j in 1:N_test){
  mu_mean <- apply(mu_test,1,mean)
  phi_test[,j] <- exp(psi_samples[,1]+psi_samples[,2]*(mu_test[,j]-mu_mean)+
                        psi_samples[,3]*(mu_samples[,j]-mu_mean)^2+
                        psi_samples[,4]*(mu_samples[,j]-mu_mean)^3)
}
phi_test
...
save(predicted_counts, file = 'output/predicted_counts_polynomial.RData')
...
pdf("output/posterior_predictive_density_mean-polynomial.pdf")
predicted_counts %>%
  apply(.,2, mean) %>%
  as.data.frame() %>%
  rename(mean = ".") %>%
...
pdf("output/posterior_predictive_density_sd-polynomial.pdf")
predicted_counts %>%
  apply(.,2, sd) %>%
  as.data.frame() %>%
  rename(sd = ".") %>%
...
pdf("output/posterior_predictive_quantile_plot-polynomial.pdf")
predicted_quantile_summary %>%
  ggplot(., aes(x = quantile, y = median)) +
  geom_point(aes(shape = "Predicted Median"), size = 2) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = "95% Interval"), alpha = 0.2) +
...
pdf("output/heatmap_mean_surface-polynomial.pdf")
mu_output_median %>%
  ggplot(., aes(x = X_coord, y = Y_coord, fill = median)) +
  geom_tile() +
  scale_fill_viridis_c() +
